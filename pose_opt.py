    ...
    for F in range(num_frames - 1):
        ...
        for k in T:
            if (F < k):
                ...
                def Pmk(
                        P_F):  # needs to calculate the derivative of P_F w.r.t. the actual image warp used on the given map template, since we are extracting the map template, and not resampling it every time
...
def test_2img(args):
    ...
    img_tens_w, mask_tens_w, _ = dlk.warp_hmg(img_tens, p_gt)
    ...
    img_tens_uw, _, _ = dlk.warp_hmg(img_tens, torch.from_numpy(P_opt).float())
...
def test_img_seq(args):
    ...
    for i in range(p_gt_comp.shape[0]):
        img_tens_w, _, _ = dlk.warp_hmg(img_tens, torch.from_numpy(p_gt_comp[i: i + 1, :, :]).float())
        I[i, :, :, :] = img_tens_w
    ...
    for i in range(num_seq):
        ...
        if (i == 0):
            ...
            I_w, _, _ = dlk.warp_hmg(I_w, torch.from_numpy(P_opt_dep[i - 1: i, :, :]).float())
...
def warp_seq(img_tens, p_gt):
    ...
    for i in range(p_gt.shape[0]):
        img_tens_w, mask_tens_w, _ = dlk.warp_hmg(I[i: i + 1, :, :, :], p_gt[i: i + 1, :, :])
...
def optimize_wmap_test(args):
    ...
    I, _ = load_I(args.image_dir, args.image_dir_ext, opt_img_height)
    ...
    M_tmpl, _ = extract_map_templates(P_mk, M, T, img_c, img_h, img_w)
    ...
    if (use_T_for_M):
        ...
        M_tmpl_tens, _, _ = dlk.warp_hmg(torch.from_numpy(M_tmpl_uw).float(), p_w)
    ...
    if (test_alignment_M_T):
        ...
        M_tmpl_lkw, _, _ = dlk.warp_hmg(M_tmpl_tens.data, p_lk.data)
    ...
    P_opt = optimize_wmap(I[0:num_seq, :, :, :], P_scale[0:num_seq - 1, :, :], T, V, P_init_scale, M_feat, T_feat, 1e-3,
                          50, 0, 1)
    ...
    for i in range(num_seq):
        if (i in T):
            ...
            M_tmpl_w, _, _ = dlk.warp_hmg(M_tmpl_curr_tens, P_mk_rel_samp_curr)
...
def sliding_window_opt(args):
    ...
    I_org, GPS_list = load_I(args.image_dir, args.image_dir_ext, opt_img_height)
    I_org_2x, GPS_list_2x = load_I(args.image_dir, args.image_dir_ext, round(scaling_for_disp * opt_img_height))
    ...
    for i in range(P_org.shape[0]):
        ...
        M_tmpl_2x, _ = extract_map_templates(P_mk, M, T, I_org_2x.shape[1], I_org_2x.shape[2], I_org_2x.shape[3])
        ...
        dlk_net = dlk.DeepLK(dlk.custom_net(args.model_path))
        max_itr_dlk = 150
        p_lk, _, itr_dlk = dlk_net(M_tmpl_tens_nmlz, T_tens_nmlz, tol=1e-4, max_itr=max_itr_dlk, conv_flag=1,
                                   ret_itr=True)
        # 修改
        # can't convert cuda:0 device type tensor to numpy. p_lk变量并不能直接转化为numpy array的格式
        p_lk = p_lk.cpu()
        p_lk2x = scale_P(p_lk.data.numpy(), scaling_for_disp)
        p_lk2x_opt, _, itr_dlk_2x = dlk_net(M_tmpl_tens_nmlz_2x, T_tens_nmlz_2x,
                                            init_param=Variable(torch.from_numpy(p_lk2x).float()), tol=1e-4,
                                            max_itr=max_itr_dlk, conv_flag=1, ret_itr=True)
        ...
        # p_lk2x_opt变量并不能直接转化为numpy array的格式
        p_lk2x_opt = p_lk2x_opt.cpu()
        ...
        xy_patch_org_cor_opt = dlk.warp_hmg_Noncentric(M, P_mk, xy_cor_curr_opt)
        ...
        distance_error_curr = 0.45 * sqrt((xy_cor_curr_gt[0]-xy_cor_curr_opt[1])^2 + (xy_cor_curr_gt[1]-xy_cor_curr_opt[0])^2)
    ...
    print("误差列表",distance_error_list)
    ...
    # 修改，保存为csv格式，未测试
    DF_data = pd.DataFrame(dict([('H_rel', H_opt_rel_coord)]))
    DF_data.to_csv(args.opt_param_save_loc, index=False)
...
def sliding_window_sz2_opt(args):
    ...
    I_org, _ = load_I(args.image_dir, args.image_dir_ext, opt_img_height)
    ...
    for i in range(P_org.shape[0] - 1):
        ...
        P_opt_i_scale = optimize_wmap(I, curr_P_scale, T, V, curr_P_init_scale, M_feat, T_feat, tol, max_itr, lam1,
                                      lam2)
...
def load_I(image_dir, ext, scaled_im_height):
    ...
    GPS_list = np.zeros((len(image_list), 2))
    ...
    for i, img_fname in enumerate(image_list):
        ...
        [lag, lag_ref, log, log_ref] = srh.get_img_log_lat(img_fname)
        GPS_list[i, :] = [lag, log]
...
if __name__ == "__main__":
    ...
    parser.add_argument('-img_h_rel_pose',
                        help='image height used for computing motion parameters, if not the native height')
